<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0">Data Laporan Keuangan - Buku Besar</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
            ...
            <div class="card card-primary">
              <div class="card-body">
                  @include('sistem.notifikasi')
                      <div class="row my-2">
                        <div class="col-md-4">
                            <form action="{{ url('laporanbukubesar')}}" method="post">
                                @csrf
                                <select name="id" class="form-control" onchange="this.form.submit();">
                                    <option value="cari">-- cari nama akun --</option>
                ...
                <tbody class="text-capitalize">
                    @if ($id <> 'cari')
                        <tr>
                            <th colspan="6">Saldo Awal Bulan</th>
                            <th>Saldo</th>
                        </tr>
                        @php
                            $saldo = 0;
                    ...
                    @foreach ($bukubesar as $item)
                        @php
                        if (($item->status_jurnalakun == 'debet' AND $item->pos_saldo == 'DEBET' ) || ($item->pos_saldo == 'KREDIT')) {
                            $saldo = $saldo + $item->nominal_jurnal;
                        } else {
                            $saldo = $saldo - $item->nominal_jurnal;
                        }
                        @endphp
                            <tr>
                                ...
                                <td class="text-right">
                                    @if ($item->status_jurnalakun == 'debet')
                                        {{ norupiah($item->nominal_jurnal)}}
                                    @endif
                                </td>
                                <td class="text-right">
                                    @if ($item->status_jurnalakun == 'kredit')
                                        {{ norupiah($item->nominal_jurnal)}}
                                    @endif
                                </td>
                                <td class="text-right">{{ norupiah($saldo)}}</td>
                            </tr>
                    @endforeach
