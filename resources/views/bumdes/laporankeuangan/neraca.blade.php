<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0">Data Laporan Keuangan - Neraca</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Data Neraca</h3>
              </div>
              <div class="card-body">
                ...
                <tbody class="text-capitalize">
                    ...
                    @endphp
                        <tr>
                            <th class="text-center">1-0000</th>
                            <th>AKTIVA</th>
                            <td></td>
                            <td></td>
                        </tr>
                    ...
                    {{-- AKTIVA TIDAK LANCAR --}}
                    @php
                        $totalaktiva    = $totalaktiva + $subtotal;
                        $subtotal = 0; 
                    @endphp
                        <tr>
                            <th class="text-center">1-2000</th>
                            <th>AKTIVA TIDAK LANCAR</th>
                            <td></td>
                            <td></td>
                        </tr>
                    @foreach ($data['aktivatidaklancar'] as $item)
                        @php
                            $jumlah  = DbHelp::totalsaldoakun($item->id,$user->id);
                            $subtotal= $subtotal + $jumlah;
                        @endphp
                    ...
                    @endforeach
                        <tr class="table-secondary">
                            <th class="text-center" colspan="3">Jumlah Aktiva Tetap</th>
                            <th class="text-right">{{ norupiah($subtotal)}}</th>
                        </tr>
                        @php
                            $totalaktiva = $totalaktiva + $subtotal; 
                        @endphp
                        <tr class="table-info">
                            <th class="text-center" colspan="3">Total Aktiva</th>
                            <th class="text-right">{{ norupiah($totalaktiva)}}</th>
                        </tr>
                    ...
                    @endphp
                        <tr>
                            <th class="text-center">2-0000</th>
                            <th>LIABILITAS</th>
                            <td></td>
                            <td></td>
                        </tr>
                    @foreach ($data['liabilitas'] as $item)
                        @php
                            $jumlah  = DbHelp::totalsaldoakun($item->id,$user->id);
                            $subtotal= $subtotal + $jumlah;
                        @endphp
                    ...
                    @endforeach
                        <tr class="table-secondary">
                            <th class="text-center" colspan="3">Jumlah Kewajiban</th>
                            <th class="text-right">{{ norupiah($subtotal)}}</th>
                        </tr>
                        {{-- @php
                            $totalaktiva = $totalaktiva + $subtotal; 
                ...
                @endphp
                    <tr>
                        <th class="text-center">3-0000</th>
                        <th>EKUITAS</th>
                        <td></td>
                        <td></td>
                    </tr>
                @foreach ($data['ekuitas'] as $item)
                    @php
                        $jumlah  = DbHelp::totalsaldoakun($item->id,$user->id);
                        // khusus untuk saldo laba berjalan
                        if ($item->kode_akun == '3-3000') {
                            $jumlah     = DbHelp::lababersihberjalan($user->id);
                        }
                        $subtotal= $subtotal + $jumlah;
                    @endphp
                    <tr>
                ...
                @endforeach
                    <tr class="table-secondary">
                        <th class="text-center" colspan="3">Jumlah Ekuitas</th>
                        <th class="text-right">{{ norupiah($subtotal)}}</th>
                    </tr>
                    @php
                        $kewajiban          = $subtotal;
                        $kewajibanekuitas = $liabilitas + $kewajiban; 
                    @endphp
                    <tr class="table-info">
                        <th class="text-center" colspan="3">TOTAL KEWAJIBAN DAN EKUITAS</th>
                        <th class="text-right">{{ norupiah($kewajibanekuitas)}}</th>
                    </tr>
